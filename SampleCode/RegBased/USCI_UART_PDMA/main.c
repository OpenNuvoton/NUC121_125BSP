...
{
    /* UUART Tx PDMA channel configuration */
    PDMA->DSCT[UUART_TX_DMA_CH].CTL =
        (UUART_TEST_LENGTH - 1) << PDMA_DSCT_CTL_TXCNT_Pos | /* Transfer count */
    ...
    PDMA->DSCT[UUART_TX_DMA_CH].SA = (uint32_t)SrcArray;     /* Source address */
    PDMA->DSCT[UUART_TX_DMA_CH].DA = (uint32_t)&UUART0->TXDAT;  /* Destination address */

    /* Request source selection */
    PDMA->REQSEL0_3 = (PDMA->REQSEL0_3 & (~PDMA_REQSEL0_3_REQSRC1_Msk)) | (PDMA_USCI0_TX << PDMA_REQSEL0_3_REQSRC1_Pos);
}
...
void PDMA_UUART_RxTest(void)
{
    ...
    PDMA->DSCT[UUART_RX_DMA_CH].CTL =
        ...
        PDMA_REQ_SINGLE  | /* Single request type */
        PDMA_OP_BASIC;     /* Basic mode */
    PDMA->DSCT[UUART_RX_DMA_CH].SA = (uint32_t)&UUART0->RXDAT;  /* Source address */
    PDMA->DSCT[UUART_RX_DMA_CH].DA = (uint32_t)DestArray;    /* Destination address */

    /* Request source selection */
    PDMA->REQSEL0_3 = (PDMA->REQSEL0_3 & (~PDMA_REQSEL0_3_REQSRC0_Msk)) | (PDMA_USCI0_RX << PDMA_REQSEL0_3_REQSRC0_Pos);
}
...
void PDMA_UUART(int32_t i32option)
{
    ...
    BuildSrcPattern((uint32_t)SrcArray, UUART_TEST_LENGTH);
    ClearBuf((uint32_t)DestArray, UUART_TEST_LENGTH, 0xFF);
    ...
    {
        UUART_TEST_LENGTH = 2;     /* Test Length */
    ...
    {
        /* Enable PDMA channel */
        PDMA->CHCTL |= ((1 << UUART_RX_DMA_CH) | (1 << UUART_TX_DMA_CH));
        ...
        PDMA->INTEN |= (1 << UUART_RX_DMA_CH);
        PDMA->INTEN |= (1 << UUART_TX_DMA_CH);
    }
    else
    {
        /* Enable PDMA channel */
        PDMA->CHCTL |= (1 << UUART_RX_DMA_CH);
